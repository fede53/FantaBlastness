                ...
                <div class="overflow-x-auto">
                    ...
                    <table id="sortable-table" class="min-w-full w-full table-auto border-collapse">
                        ...
                        <tr>
                            ...
                            <th data-column="name" data-order="asc" class="w-4/12 px-4 py-3 text-left text-white font-bold cursor-pointer bg-dark">
                                {{ __('Name') }}
                            </th>
                            <th data-column="email" data-order="asc" class="w-3/12 px-4 py-3 text-left text-white font-bold cursor-pointer bg-dark">
                                {{ __('Email') }}
                            </th>
                            <th data-column="role" data-order="asc" class="w-3/12 px-4 py-3 text-left text-white font-bold cursor-pointer bg-dark">
                                {{ __('Role') }}
                            </th>
                            <th class="w-3/12 px-4 py-3 text-left text-white font-bold bg-dark">{{ __('Actions') }}</th>
                        </tr>
                        </thead>
                        <tbody>
                        @forelse ($users as $user)
                            <tr class="bg-dark-100 hover:bg-dark-100 transition">
                                ...
                                <td class="px-4 py-3 text-white">{{ $user['surname'] }} {{ $user['name'] }}</td>
                                <td class="px-4 py-3 text-white">{{ $user['email'] }}</td>
                                <td class="px-4 py-3 text-white">{{ $user['role'] }}</td>
                                <td class="px-4 py-3 space-x-2">

                                    <div class="flex items-center justify-center">
                                        <!-- Pulsante Modifica -->
                                        <a href="{{ route('users.edit', $user['id']) }}" class="text-white hover:text-white button-action" title="{{ __('Edit') }}">
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-4 py-3 text-center text-white">{{ __('No users found.') }}</td>
                            </tr>
                        @endforelse
                        </tbody>
                    </table>
